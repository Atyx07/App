...
with st.sidebar:
    ...
    if py_version.startswith("3.11"):
        ...
        st.error(f"ERREUR: Python doit être 3.11. (Votre build a échoué)")
    ...
    if st_version == "1.29.0":
        ...
        st.error(f"ERREUR: Streamlit doit être 1.29.0. (Votre build a échoué)")
...
st.info("Utilisez la flèche `>` en haut à gauche pour ouvrir le panneau de débogage.")
st.divider()
